...
public interface IRequestAttendRepository extends JpaRepository<RequestAttendEntity, Long>, JpaSpecificationExecutor<RequestAttendEntity> {
    ...
    @Query(value = "SELECT NEW cy.dtos.attendance.RequestAttendDto(r) FROM RequestAttendEntity r WHERE r.createBy.userId = ?1")
    ...
    @Query(value = "SELECT NEW cy.dtos.attendance.RequestAttendDto(r) FROM RequestAttendEntity r WHERE r.id = ?1")
