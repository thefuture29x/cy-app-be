...
public interface IBugRepository extends JpaRepository<BugEntity, Long> {
    ...
    @Query(value = "SELECT * FROM `tbl_bugs` \n" +
            "WHERE id IN (SELECT bg.id FROM `tbl_bugs` bg " +
            "JOIN `tbl_tasks` ts ON bg.task_id = ts.id " +
            "JOIN `tbl_features` ft ON ts.feature_id = ft.id \n" +
            "WHERE ft.id = ?1 AND bg.is_deleted = 0 ) " +
            "OR id IN (SELECT bg.id FROM `tbl_bugs` bg JOIN `tbl_sub_tasks` sts ON bg.sub_task_id = sts.id " +
            "JOIN `tbl_tasks` ts ON sts.task_id = ts.id\n" +
            "JOIN `tbl_features` ft ON ts.feature_id = ft.id WHERE ft.id = ?1 AND bg.is_deleted = 0)", nativeQuery = true)
    Page<BugEntity> findAllBugOfFeature(Long idFeature, Pageable pageable);

    @Query(value = "SELECT * FROM tbl_bugs bg WHERE bg.task_id=?1 AND bg.is_deleted =0",nativeQuery = true)
