...
public interface ITaskRepository extends JpaRepository<TaskEntity, Long> {
    ...
    @Query(value = "SELECT tas.* FROM `tbl_projects` pro \n" +
            ...
            "WHERE pro.id = ?1\n" +
            "AND tas.is_deleted = 0 AND fea.is_deleted = 0 AND pro.is_deleted = 0\n" +
            "AND tas.id not in (SELECT DISTINCT task_id FROM tbl_sub_tasks WHERE is_deleted = 0)\n", nativeQuery = true)
    ...
    @Query(value = "SELECT is_deleted FROM `tbl_tasks` WHERE id = ?1", nativeQuery = true)
